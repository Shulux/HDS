...
# convert the continuous volume variable to a factor (categorical variable)
...
trees$Height_scaled <- as.numeric(scale(trees$Height, center = TRUE, scale = TRUE))
trees$Girth_scaled <- as.numeric(scale(trees$Girth, center = TRUE, scale = TRUE))

str(trees)
head(trees)


tree_pca <- prcomp(~Height_scaled + Girth_scaled,data = trees)
summary(tree_pca)
...
load(file="Practicals/tumor_tissue.rda")
...
Z <- svd_X$u %*% diag(svd_X$d) #calculate and store the scores (PCs)

#PCA
pca_x <- prcomp(X, center=FALSE, scale.=FALSE)#no need to standardise here as the variables were already scaled
summary(pca_x)
pca_x$x #scores or PCs - same as Z in the above obtained directly using SVD
