...
for(i in 1:6) {
  ...
  km.out3 <- kmeans(x=Tumor, centers=3, iter.max=100, nstart=1)
  # Plot clusters
  plot(x=Tumor, col=km.out3$cluster, main=km.out3$tot.withinss)
}
...
for (i in 1:10) {
  km.outi <- kmeans(x=Tumor, centers=i, iter.max=100, nstart=20)
  # Save total within sum of squares to wss variable
  wss[i] <- km.outi$tot.withinss
...
load(file="Practicals/SP500data.rda")
...
k <- 3
km.out <- kmeans(x=SP500data, centers=k, iter.max=100, nstart=20)
